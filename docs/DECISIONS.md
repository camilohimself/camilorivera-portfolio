...
## 13 septembre 2026 — Relier le cahier

- **Contexte** : le maillage interne est relevé page par page avant d’être touché. Six constats mesurés : l’accueil se terminait sur deux adresses mail, sans aucun lien interne dans son pied de page ; le bloc de page précédente et suivante n’existait que sur quatre chapitres et bouclait sur lui-même ; la réserve et Hors cadre, les deux pages les plus fournies, n’avaient ni précédent ni suivant ; cinq pages conduisaient à Hors cadre par des ancres précises sans qu’il renvoie vers aucune d’elles ; les échos éditoriaux ne servaient que dans un sens ; le sommaire répétait quatre fois la même invitation.
- **Le point de départ** : `js/motion.js` porte depuis l’origine une liste des huit pages dans l’ordre, dont `setDirection()` tire le sens de la feuille qui se tourne. C’était la seule déclaration d’un ordre de lecture du site, et le maillage ne la suivait pas jusqu’au bout. Le chantier fait dire la même chose aux liens et à l’animation.
- **Décision** : le cahier se ferme plutôt qu’il ne boucle. Les six chapitres s’enchaînent des carnets à Hors cadre ; la case gauche des carnets ramène au sommaire, la case droite de Hors cadre porte « Sortir de l’atelier » vers la collection. Vérifié au navigateur : `data-page-way` vaut `forward` en avançant et `back` en reculant.
- **Hors cadre rend la monnaie** : ses quatre sections correspondent aux chapitres qui y mènent, cinq passerelles numérotées repartent donc vers eux. Son fil d’Ariane passe de deux à trois échelons et son folio prend la forme des cinq autres, « 06 / 06 ».
- **Deux composants changent de feuille** : `.journal-echo` et `.hc-nav-entry` quittent `css/reverie.css` pour `css/intensity.css`, chargée par les huit pages. Ils servent désormais aussi depuis Hors cadre, qui ne charge pas `reverie.css`. C’est le trajet inverse de celui du 12 septembre, motivé par ce nouvel usage ; leurs réglages mobiles restent dans une media query de même seuil.
- **La paire manquante** : les carnets et les caves n’étaient reliés dans aucun sens. Un bandeau « 03 — Jusque sur un mur » part des carnets, un écho « Avant le mur, la page. » revient des caves, sur une page qui n’en portait aucun.
- **Six verbes au lieu d’un** : « Ouvrir cette page », répété quatre fois, devient ouvrir, approcher, descendre, ramasser ; fouiller et entrer étaient déjà en place. Le verbe « entrer » servant déjà cinq fois ailleurs, il n’en a pas été ajouté un sixième.
- **Textes neufs** : le seuil de fin d’accueil et les six phrases de passage sont écrits pour ce chantier. Ils relèvent du registre éditorial du site, celui de « Rien n’oblige à commencer par le début. », et non de la parole de Camilo : aucune de ses formulations n’est réécrite, aucune première personne n’est inventée. Français et anglais sont posés dans le même commit.
- **Écarté** : faire boucler Hors cadre sur les carnets, qui rendrait le parcours sans fin ; empiler un bloc de navigation de plus sur les finales existantes de la réserve et de Hors cadre, déjà fortes ; ajouter « Écrire à Camilo » au pied de page de l’accueil, où le contact figure déjà dans la navigation et le dock — seul « Le journal » y manquait vraiment. Le folio des chapitres n’a pas été créé : cinq d’entre eux le portaient déjà, seul Hors cadre divergeait.
- **Contrôles** : `node tools/validate-journal.mjs` (8 pages, 987 références locales, aucune ancre manquante), `node tools/validate-portfolio.mjs`, `git diff --check`. Parcours complet suivi au navigateur, de l’accueil à la collection, sans utiliser la navigation de l’en-tête. Aucun débordement horizontal sur les huit pages à 390 px.
- **Branche** : `claude/maillage`, depuis `origin/main` à `9ec295f`.
- **Tranché par** : Camilo, qui a choisi la fermeture du cahier, le périmètre et l’ordre des étapes sur la carte du maillage.
...
- **Branche** : commit `67867dc` sur `codex/art-digital`, reporté sur `main` pour la publication Netlify ; seuls README, DECISIONS et VERIFICATION ont demandé une résolution manuelle.
