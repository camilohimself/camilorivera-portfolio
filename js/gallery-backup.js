    ...
    artworks.forEach((artwork, index) => {
        ...
        card.innerHTML = `
            <img src="${artwork.image}"
                 ...
                 onerror="console.error('Erreur chargement:', this.src); this.style.background='linear-gradient(135deg, #e0e0e0 0%, #f5f5f5 100%)'; this.style.opacity='1';"
        ...
        // Force image visibility after a short delay as fallback
        const img = card.querySelector('.painting-image');
        if (img) {
            setTimeout(() => {
                if (img.style.opacity !== '1') {
                    console.log('🔧 Forçant la visibilité de l\'image:', artwork.image);
                    img.style.opacity = '1';
                }
            }, 2000);
        }
